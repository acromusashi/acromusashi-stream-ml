...
/**
 * KMeans分散用Infinispan向けStateクラス<br>
 * 設定された「parallelismHint」の数だけ存在する。
 * 
 * @author kimura
 */
public class InfinispanKmeansState extends KmeansState
{
    /** キャッシュ上のデータ生存期間のデフォルト値 */
    private static final int                               DEFAULT_LIFESPAN = 600;

    /** logger */
    private static final Logger                            logger           = LoggerFactory.getLogger(InfinispanKmeansState.class);
     ...
     */
    public InfinispanKmeansState(int partitionIndex, int numPartitions, String targetServer,
            String cacheName)
    {
     ...
     */
    @Override
    protected KmeansDataSet getState(String baseKey, Long txId, int partitionIndex,
            boolean isSelfPartition) throws IOException
    {
        ...
        KmeansDataSet targetValue = this.stateCache.get(targetKey);
        return targetValue;
    }
     ...
     */
    @Override
    protected void putState(String baseKey, Long txId, int partitionIndex, KmeansDataSet putState)
            throws IOException
    {
